...
# Prediction function (to be run in a separate thread)
def predict():
...
while running:
    ...
    with prediction_lock:
        if current_prediction:
            print(f"[PREDICT] Updated current_prediction to: {current_prediction}")
            # Ensure some time has passed since the last prediction before applying another action
            current_time = pygame.time.get_ticks()
            if prediction_timestamp is None or current_time - prediction_timestamp > 500:  
                if current_prediction == 'Rotating':
                    rotation_x += 1
                elif current_prediction == 'rotate_x_negative':
                    rotation_x -= 1
                elif current_prediction == 'rotate_y_positive':
                    rotation_y += 0.1
                elif current_prediction == 'rotate_y_negative':
                    rotation_y -= 0.1
                elif current_prediction == 'zoom_in':
                    zoom += 2
                elif current_prediction == 'Blank':
                    zoom = max(5, zoom - 2)
