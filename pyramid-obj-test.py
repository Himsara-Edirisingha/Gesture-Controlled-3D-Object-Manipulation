...
pygame.display.set_caption("Interactive Node Pyramid with Enhanced Rendering")
...
class Pyramid:
    def __init__(self, base_size, height, resolution):
    ...
    def create_nodes(self):
        nodes = []
        for h in np.linspace(0, self.height, self.resolution):  # Fill the pyramid with nodes
            scale = (self.height - h) / self.height
            for x in np.linspace(-self.base_size * scale / 2, self.base_size * scale / 2, self.resolution):
                for y in np.linspace(-self.base_size * scale / 2, self.base_size * scale / 2, self.resolution):
                    position = np.array([x, y, h - self.height / 2])
...
# Pyramid dimensions
pyramid_base_size = 10
pyramid_height = 10
pyramid_resolution = 15
pyramid = Pyramid(pyramid_base_size, pyramid_height, pyramid_resolution)
...
while running:
    ...
    if dissected:
        ...
        pyramid.draw(screen, zoom, offset_x, offset_y, rotation_matrices, dissection_mode, mouse_x, mouse_y, dissected)
