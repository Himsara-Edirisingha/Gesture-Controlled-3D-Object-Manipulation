        ...
        for x in range(self.width):
            for y in range(self.height):
                for z in range(self.length):
                    ...
                    nodes.append(Node(position, color=(255, 255, 255, 255)))  # Default color green
        return nodes
...
class CuttingPane:
    def __init__(self, axis, initial_position=0):
        self.axis = axis  # Axis of the cutting plane x y z
        self.position = initial_position  # Position of the cutting plane along the axis

    def draw(self, screen, zoom, offset_x, offset_y, rotation_matrices):
        # Create the visual representation of the cutting pane
        if self.axis == 'x':
            pane_color = (255, 0, 0, 128)  # Semi-transparent red for X-axis
            plane_points = [
                np.array([self.position, -10, -10]),
                np.array([self.position, 10, -10]),
                np.array([self.position, 10, 10]),
                np.array([self.position, -10, 10])
            ]
        elif self.axis == 'y':
            pane_color = (0, 255, 0, 128)  # Semi-transparent green for Y-axis
            plane_points = [
                np.array([-10, self.position, -10]),
                np.array([10, self.position, -10]),
                np.array([10, self.position, 10]),
                np.array([-10, self.position, 10])
            ]
        elif self.axis == 'z':
            pane_color = (0, 0, 255, 128)  # Semi-transparent blue for Z-axis
            plane_points = [
                np.array([-10, -10, self.position]),
                np.array([10, -10, self.position]),
                np.array([10, 10, self.position]),
                np.array([-10, 10, self.position])
            ]
        pane_width = 2  # Thickness of the pane

        # Rotate and project the plane points
        projected_points = []
        for point in plane_points:
            rotated_point = point.dot(rotation_matrices[0]).dot(rotation_matrices[1]).dot(rotation_matrices[2])
            projected_point = rotated_point[:2] * zoom
            screen_x = int(projected_point[0] + offset_x)
            screen_y = int(projected_point[1] + offset_y)
            projected_points.append((screen_x, screen_y))

        # Draw the cutting pane as a polygon
        pygame.draw.polygon(screen, pane_color, projected_points, pane_width)

        # Display the current position of the cutting plane
        font = pygame.font.Font(None, 24)
        text = font.render(f"{self.axis.upper()}-Plane Position: {self.position:.2f}", True, pane_color[:3])
        screen.blit(text, (10, 10 + 30 * (ord(self.axis) - ord('x'))))
...
# Create cutting pane objects for each axis
cutting_panes = {
    'x': CuttingPane('x'),
    'y': CuttingPane('y'),
    'z': CuttingPane('z')
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif event.key == pygame.K_z:  # Move cutting plane up along the Z-axis
                cutting_panes['z'].update_position(1)
            elif event.key == pygame.K_x:  # Move cutting plane down along the Z-axis
                cutting_panes['z'].update_position(-1)
            elif event.key == pygame.K_c:  # Move cutting plane up along the X-axis
                cutting_panes['x'].update_position(1)
            elif event.key == pygame.K_v:  # Move cutting plane down along the X-axis
                cutting_panes['x'].update_position(-1)
            elif event.key == pygame.K_b:  # Move cutting plane up along the Y-axis
                cutting_panes['y'].update_position(1)
            elif event.key == pygame.K_n:  # Move cutting plane down along the Y-axis
                cutting_panes['y'].update_position(-1)
    ...
    if dissection_mode:
        cube.dissect(cutting_panes)

    # Draw the cube nodes and lines
    cube.draw(screen, zoom, offset_x, offset_y, rotation_matrices, dissection_mode, mouse_x, mouse_y)

    # Draw the cutting panes
    if dissection_mode:
        for pane in cutting_panes.values():
            pane.draw(screen, zoom, offset_x, offset_y, rotation_matrices)
